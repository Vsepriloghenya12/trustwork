...
const { resolveReviewTarget } = await import('../src/services/reviews.js')
...
test('до завершения сделки фрилансер оставляет отзыв без звезд (DIALOG)', () => {
...
test('заказчик оценивает исполнителя после завершения: звезды, без диалог-гейта', () => {
  const project = { status: 'COMPLETED', clientId: 'c1', freelancerId: 'f1' }
